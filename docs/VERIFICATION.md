...
- Cancellation reaches detached accessibility/OCR workers and prevents later stages from publishing abandoned captures. Clipboard restoration reports failures, preserves newer copies, and survives repeated cancellation. Copy-collapse tests use controlled deadlines, with a generation guard against stale callbacks.

## Verification

- **378 tests passed with zero failures or skips** in both local debug and optimized release configurations. Coverage includes clipboard payloads and restoration failures, capture cancellation, selection, display topology changes, preserved native panel state, interaction deferral, and capture-card completion outside display bounds. The native-motion CI skip is removed: animated transitions use controlled time and accessibility inputs while retaining the real native window and display link, and a separate frozen-time test verifies Reduce Motion. CI rejects skipped tests.
- Issue #2 was a test assumption: the [GitHub runner image enables Reduce Motion](https://github.com/actions/runner-images/blob/691bdc2593839b6bd2615aaa9718ca646ec3d4a6/images/macos/scripts/build/configure-system.sh#L19-L21). A diagnostic run confirmed the panel reached the correct 440-point Settings height before the test timed out waiting for an intermediate frame; the later `InvalidTransition` message obscured that timeout. Production motion timing and accessibility defaults remain unchanged.
- The packaged **0.6.0 (29) Preview**, source `5fd8876c4e9dbc1eaba4b4dd215965db6b5f6f77`, was signed, certificate/OCSP-verified, installed, and inspected through computer use. Settings confirms the exact build and both capture permissions Allowed. Theme selection was changed and restored; display following toggled on/off; all three copy choices selected correctly. Scrollbar End/Home moved to 100/0 percent, confirming the earlier keyboard-focus fix.
- A physical Shift + Shift capture was confirmed by Brian. Two real shots were selected and copied through the shelf, which collapsed after copying. Pasting into TextEdit produced **two PNG attachments plus the complete 53,463-character copied context**; the saved local RTFD was checked for both images and exact context inclusion. This verifies combined native rich copy end to end. Temporary QA settings were restored, and the previous app bundle is preserved locally.
- Physical clear-all, pointer-driven movement between monitors, cross-display capture choreography, and assisted image-then-text delivery into web composers were not repeated in this pass. Automated native/policy tests are not a substitute for those physical-input and receiver checks. Destination behavior remains app-dependent.
...
- The 0.6.0 app and matching source ZIP are certificate/archive-verified and accompanied by SHA-256 checksums and a release manifest. GitHub and the website publish the same bytes; immutable 0.5.2 downloads remain available. The source archive records the app build commit even though this publication documentation comes later.
